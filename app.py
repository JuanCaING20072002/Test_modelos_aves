...
if nav.startswith("🔍"):
    # Controles de predicción (versión única y ordenada)
    st.sidebar.markdown("---")
        ...
        if not HAS_TF or load_img is None:
            ...
            try:
                ...
                if probs_all is None:
                    raise RuntimeError("predict_proba devolvió None")
    ...
    # Si no hay probabilidades válidas, evitamos renderizar Top-K
    if probs.size == 0:
        st.info("Sube una imagen válida para ver predicciones Top-K.")
    else:
        ...
        # Top-K visual: imágenes en columnas con botón debajo (único bloque)
        ...
        # Mostrar métricas + imágenes en columnas
        with c2:
            ...
            # Renderizar cada opción en su columna con botón debajo
        ...
        # Imagen de referencia principal (a la derecha de la selección)
